...
// hoiiiiiiaisdifaisdfiasdifiasdfasdfasdfasdf
...
gsap.set('.eventcontent img',{xPercent:-50, yPercent:-50})

const imgs = gsap.utils.toArray("#event .eventcontent");
...
// gsap.set('.eventcontentimg img',{xPercent:-50, yPercent:-50})

const imgsimg = gsap.utils.toArray("#event .eventcontentimg");
const nextimg = 5; // time to change
const fadeimg = 1.5; // fade time 
//only for the first 
gsap.set(imgsimg[0], {autoAlpha:1})

function crossfadeimg(){

  const action = gsap.timeline()
  .to(imgsimg[0], {autoAlpha:0, duration:fadeimg})
  .to(imgsimg[1], {autoAlpha:1, duration:fadeimg},0)

  imgsimg.push( imgsimg.shift() );

  gsap.delayedCall(nextimg, crossfadeimg);
}
